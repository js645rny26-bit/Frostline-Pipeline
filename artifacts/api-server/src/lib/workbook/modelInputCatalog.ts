...
const ENTRIES: ModelInputCatalogEntry[] = [
  ...
  SOURCE(
    ...
    "Season xwOBA is active. The endpoint does not currently expose the hard-hit field required by Patch B; damage remains fail-closed and shadow-only.",
    "SAVANT_BATTER_SEASON",
  ),
  SOURCE(
  ...
  SOURCE(
    ...
    "EVERY_PREGAME_RUN",
    "SOURCE_ACQUISITION_LOG raw hash/data-through/schema/MLBAM coverage + SWE_APPEARANCE_HISTORY_V1 + BVH_DAILY_HISTORY_V1 + BATTER_DAMAGE_DAILY_V1",
    "Raw daily events are retained before workload, PA-reduced batter-vs-hand, or batted-ball damage engineering. Every consumer retains its own cutoff and status lineage.",
    "SAVANT_PITCH_LEVEL",
  ),
  SOURCE(
  ...
  },
  {
    ...
    correlationFamily: "STARTER_WORKLOAD", missingBehavior: "Unresolved identity remains unresolved; unavailable history uses an explicit listed-starter prior and never a postgame reconstruction.",
    notes: "Role identity and numeric workload are separate. Workload is not a generic pitcher-success label. OPENER/BULK requires explicit pregame role evidence; recent pitch-count magnitude cannot create the role.", freshnessKey: "STARTER_WORKLOAD",
  },
  {
    recordType: "INPUT", id: "SWE_V1_STARTER_WORKLOAD_SHADOW", label: "Starter Workload V1 frozen lineage / legacy replay",
    layer: "BASEBALL_MODEL", outputClass: "DIAGNOSTIC", operationalStatus: "ACTIVE",
    definition: "Frozen PITCHER_SPECIFIC_WORKLOAD_CANDIDATE_V1 lineage for the estimator now consumed by Module 03, retained under its original version label so historical legacy-versus-candidate replay remains auditable.",
    statisticalWindow: "Up to five prior MLB starts through slate date minus one", gameWindow: "STARTER_WINDOW -> BULLPEN_WINDOW + SETTLEMENT REPLAY",
    primarySource: "SOURCE_MLB_STARTER_WORKLOAD", fallbackSource: "Explicit ROLE_FALLBACK_NO_USABLE_HISTORY; role/return prior only",
    refreshCadence: "EVERY_PREGAME_RUN", freshnessEvidence: "PREGAME_PACKET_HISTORY Workload_Candidate_* + Away/Home_Projected_IP_Shadow",
    workbookLocation: "PREGAME_PACKET_HISTORY Workload_Candidate_*; STARTER_OUTCOME_DIAGNOSTICS Projected_IP_Shadow; SWE_WORKLOAD_DEVIATION_V1; SWE_WORKLOAD_REPLAY_SUMMARY_V1", feedsActiveProjection: "YES", feedsDecisionBoard: "YES",
    correlationFamily: "STARTER_WORKLOAD", missingBehavior: "No usable source history creates ROLE_FALLBACK_NO_USABLE_HISTORY; Module 30 fails closed rather than substituting old SWE_* or active Expected_IP.",
    notes: "Schema v75 commissions the unchanged estimator only to repair role/workload coupling. Module 30 continues to preserve historical legacy comparisons. Older Module 02i SWE_* fields remain a separate shadow and are not an additional active vote.", freshnessKey: "STARTER_WORKLOAD",
  },
  {
  ...
  {
    recordType: "INPUT", id: "SAVANT_SEASON_CONTACT", label: "Season xwOBA and optional damage fields",
    layer: "BASEBALL_MODEL", outputClass: "ACTIVE_INPUT", operationalStatus: "ACTIVE",
    definition: "Season Statcast xwOBA contributes to active lineup quality. Optional hard-hit data is schema-audited separately and has no active consumer while Patch B is gated.",
    ...
    correlationFamily: "LINEUP_QUALITY / DAMAGE", missingBehavior: "xwOBA degrades to OPS-only lineup quality; absent damage columns remain an explicit source-contract gap and cannot silently become a neutral observed value.",
    notes: "The current endpoint materializes xwOBA but not hard-hit rate. xBA/xSLG may materialize; barrel rate and exit velocity are optional and none of those optional fields is active.", freshnessKey: "SAVANT_BATTER_SEASON",
  },
  {
  ...
  },
  {
    ...
    definition: "Candidate lineup hard-hit rate versus starter HR/9 interaction for bounded signed damage runs. The active caller is fail-closed while its cutoff-safe source contract is repaired and commissioned.",
    statisticalWindow: "Season to date", gameWindow: "STARTER_WINDOW -> BULLPEN_WINDOW",
    primarySource: "SOURCE_SAVANT_PITCH_LEVEL + SOURCE_MLB_PITCHER_SEASON", fallbackSource: "NEUTRAL active component; existing preview collision remains a separate shadow",
  ...
  },
  {
    ...
    layer: "SETTLEMENT", outputClass: "DIAGNOSTIC", operationalStatus: "SHADOW_ONLY",
    definition: "Price-blind labels for opener-chain uncertainty, starter-versus-bullpen path dependence, traffic/damage co-signing, and asymmetric traffic-only or damage-only support, derived only from the packet already frozen for settlement.",
    ...
    workbookLocation: "FAILURE_CLASSIFICATION_SHADOW_V1; FAILURE_CLASSIFICATION_REPLAY_V1; FAILURE_CLASSIFICATION_DISCRIMINATION_V1", feedsActiveProjection: "NO", feedsDecisionBoard: "NO",
