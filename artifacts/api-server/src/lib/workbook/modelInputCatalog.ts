...
const ENTRIES: ModelInputCatalogEntry[] = [
  ...
  },
  {
    ...
    definition: "Frozen PITCHER_SPECIFIC_WORKLOAD_CANDIDATE_V1 expected innings from up to five MLB game-log appearances, 0.50/0.30/0.20/0.10/0.05 recency weighting, and min(n/5,1) shrinkage toward role/return priors.",
    statisticalWindow: "Up to five role-relevant MLB game-log appearances through slate date minus one", gameWindow: "STARTER_WINDOW -> BULLPEN_WINDOW (REPLAY ONLY)",
    ...
    workbookLocation: "PREGAME_PACKET_HISTORY Workload_Candidate_*; STARTER_OUTCOME_DIAGNOSTICS Projected_IP_Shadow; SWE_WORKLOAD_DEVIATION_V1; SWE_WORKLOAD_REPLAY_SUMMARY_V1", feedsActiveProjection: "NO", feedsDecisionBoard: "NO",
    ...
    notes: "Conventional personalization is tested as frozen Projected_IP_Shadow versus actual deviation from the 6.0-IP role baseline, with N=150 plus a separately frozen outcome-variance gate. Opener/bulk evidence is a distinct hypothesis. Older Module 02i SWE_* fields are not the evaluated candidate. No projection/survival/collision/authorization consumer exists.", freshnessKey: "STARTER_WORKLOAD",
  },
  {
  ...
  },
  {
    ...
    workbookLocation: "BATTER_DAMAGE_DAILY_V1; BATTER_DAMAGE_PROFILES_V1; DAMAGE_LINEUP_SHADOW_V1; STATCAST_SHADOW_AUDIT; GAME_SUMMARY reserved damage fields", feedsActiveProjection: "NO", feedsDecisionBoard: "NO",
