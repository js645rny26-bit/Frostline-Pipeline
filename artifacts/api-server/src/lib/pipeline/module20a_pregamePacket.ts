...
import type { SWEGameState } from "./module02i_starterWorkloadEstimator.js";
...
export const PREGAME_PACKET_HISTORY_HEADERS = [
  ...
  "Home_Workload_Notes",
  // Starter Workload Estimator V1. This source-only object is frozen for
  // innings replay; no active projection or decision consumer may read it.
  "SWE_Version",
...
export function buildPregamePacketInputs(
  ...
  sweStatesByGame: ReadonlyMap<string, SWEGameState> = new Map(),
): PregamePacketInput[] {
  ...
  return summaries.flatMap((summary) => {
    ...
    const values: unknown[] = [
      ...
      workloadState?.away.workload_candidate_version
        ?? workloadState?.home.workload_candidate_version
        ?? "PITCHER_SPECIFIC_WORKLOAD_CANDIDATE_V1",
      workloadState?.away.workload_candidate_status ?? "ROLE_FALLBACK_NO_USABLE_HISTORY",
      blank(workloadState?.away.projected_pitches_shadow),
      workloadState?.away.workload_data_through_date ?? "",
      blank(workloadState?.away.workload_relevant_appearances),
      blank(workloadState?.away.workload_recent_ip),
      blank(workloadState?.away.workload_recent_pitches),
      blank(workloadState?.away.workload_ip_sd),
      blank(workloadState?.away.workload_pitch_sd),
      blank(workloadState?.away.workload_history_weight),
      ...
      workloadState?.home.workload_candidate_status ?? "ROLE_FALLBACK_NO_USABLE_HISTORY",
      blank(workloadState?.home.projected_pitches_shadow),
      workloadState?.home.workload_data_through_date ?? "",
      blank(workloadState?.home.workload_relevant_appearances),
      blank(workloadState?.home.workload_recent_ip),
      blank(workloadState?.home.workload_recent_pitches),
      blank(workloadState?.home.workload_ip_sd),
      blank(workloadState?.home.workload_pitch_sd),
      blank(workloadState?.home.workload_history_weight),
      ...
      "1.0.0",
      blank(sweState?.away.expected_ip),
      sweState?.away.status ?? "INSUFFICIENT_HISTORY",
      sweState?.away.n_starts ?? 0,
      blank(sweState?.away.l3_ip),
      blank(sweState?.away.l5_ip),
      blank(sweState?.away.season_ip),
      blank(sweState?.away.shrinkage_weight),
      blank(sweState?.home.expected_ip),
      sweState?.home.status ?? "INSUFFICIENT_HISTORY",
      sweState?.home.n_starts ?? 0,
      blank(sweState?.home.l3_ip),
      blank(sweState?.home.l5_ip),
      blank(sweState?.home.season_ip),
      blank(sweState?.home.shrinkage_weight),
...
export function upsertPregamePacketRows(
  ...
  for (const input of incoming) {
    ...
    if (input.packet_status === "FROZEN_PREGAME")
      next[I.SWE_Snapshot_Primary] = "YES";
...
export function finalizeOpenPregamePacketRows(
  ...
  for (const [rowIndex, existing] of rows.entries()) {
    ...
    frozen[I.SWE_Snapshot_Primary] = "YES";
...
export async function writePregamePacketHistory(
  ...
  options: {
    ...
    sweStatesByGame?: ReadonlyMap<string, SWEGameState>;
  } = {},
