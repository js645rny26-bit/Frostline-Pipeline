...
- **Pass A integrity repairs (2026-07-25)**: Six fixes applied atomically. (1) `BOARD_LOCK_LATE_GRACE_MS=30min` in config — first lock >30min after cutoff defaults LOCKED_OUT, not retroactive LOCKED_IN. (2) module17 `logVehicles` now UPSERTs (overwrites today's rows) instead of insert-only, with cross-date game_id validation (game_id date prefix must match `date` param). (3) runner.ts: module11 failure forces `partial_success` + named error in allErrors. (4) module10: writes correct SLATE_INPUT headers (A=Game_ID…AH=Board_Lock_Status) every publish; clears stale prior-date rows after writing. (5) module17 `runPostmortem`: filters contaminated rows (game_id date ≠ row date) before grading. All compile clean; publish verified success with 15 games, 0 errors.
- **Step 6 replay validation (commissioning step 6, 2026-07-24)**: module13 extended with BLEND_PARK_PITCHER variant (6th variant). Two-phase loop: phase 1 = collect games+rates per date; phase 2a = batch-fetch game boxscores via /game/{pk}/boxscore (NOT schedule hydrate=boxscore which returns empty); phase 2b = batch-fetch pitcher stats; phase 3 = compute. **REPLAY VERDICT (30 days, 356 games):** BLEND_PARK MAE=3.702, BLEND_PARK_PITCHER MAE=3.716 (+0.014, REGRESSION). Pitcher quality at K_BB_BLEND_WEIGHT=0.70 does NOT improve accuracy — miss4+ increases 2.5pp. **Decision: do NOT apply pitcher quality factor to live run projection formula.** Keep as informational columns (GAME_SUMMARY AF:AH, SLATE_BOARD Z:AA) only. BLEND_PARK is the canonical projection variant. All blend weights remain provisional until full-season replay.
