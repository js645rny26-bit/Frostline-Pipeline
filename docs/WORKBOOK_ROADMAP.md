...
| `REGRESSION_REPORT`                 | After settlement, Module 15  | MAE, median error, bias, miss rate, and projection direction summaries.                                                         | Reliability evidence only.                                         | `Bias_Alert` means only that absolute mean signed error exceeds 0.20 runs for that window. It has no sample floor or uncertainty test and is not a model-condition verdict. Separate total accuracy from allocation and winner accuracy. |
