...
| `ACTIVE_PITCHING_INVENTORY_V1`      | Legitimate pre-first-pitch publish, Module 36 | Named starter plus source-supported inferred bulk/swing candidates, availability, workload provenance, and starter/bulk/true-bullpen phase allocation. | Research only; `Active_Input=NO` and no Module 09 or decision consumer exists. | `PROBABLE_INFERRED` is not an announced follower. Never reconstruct a missing pregame chain from postgame pitcher order. |
