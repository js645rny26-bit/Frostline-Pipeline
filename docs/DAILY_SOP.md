...
- **Active math:** `GAME_SUMMARY` uses the exact confirmed/prospective lineup against the opposing starter to separate traffic/conversion, expected starter workload, and inherited bullpen exposure. The HR/XBH damage component is currently fail-closed at zero behind the Patch B commissioning gate because its expected-statistics source does not materialize hard-hit rate; the non-zero damage estimate in `STATCAST_SHADOW_AUDIT` remains research-only. Read `*_Matchup_Profile_Status` first: `NEUTRAL` means the formula intentionally preserved the prior rate/quality path because exact evidence was unavailable.
...
- **Settlement:** run the daily settlement workflow for the completed slate
  ...
  Module 37 starter-window support then uses Module 32's exact batting-side starter-window outcomes
...
- **Lineups / starter fallback:** `official` beats `projected` — the status is per game in TODAY_LINEUPS. Platoon numbers are computed from whatever lineup is posted. The individual MLB Starting Nine team page may fill a probable starter only when MLB has left that exact game/team slot unresolved and the page's MLBAM identity verifies; it never overwrites an MLB probable starter. Its split, season, park, and umpire statistics are published in `STARTING_NINE_TEAM_PAGE_V1` as display-only evidence and do not feed active projections. Exact mutable-game page responses are retained in the existing append-only source ledgers; the current-state display sheet is not a chronology ledger.
